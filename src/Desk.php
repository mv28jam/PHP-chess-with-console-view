...
class Desk
{
     ...
     * Set figures
     */
    public function __construct(stdClass $last_game_data = null)
    {
        if (!is_null($last_game_data)) {
            foreach ((array)$last_game_data as $block) {
                foreach ($block as $figure) {
                    switch ($figure->name) {
                        case('Rook'):
                            $this->figures[$figure->x][$figure->y] = new Rook($figure->is_black);
                            break;
                        case('Knight'):
                            $this->figures[$figure->x][$figure->y] = new Knight($figure->is_black);
                            break;
                        case('Bishop'):
                            $this->figures[$figure->x][$figure->y] = new Bishop($figure->is_black);
                            break;
                        case('Queen'):
                            $this->figures[$figure->x][$figure->y] = new Queen($figure->is_black);
                            break;
                        case('King'):
                            $this->figures[$figure->x][$figure->y] = new King($figure->is_black);
                            break;
                        case('Pawn'):
                            $this->figures[$figure->x][$figure->y] = new Pawn($figure->is_black);
                            break;
                        default:
                            break;
                    }
    ...
    public function toMap(): array
    {
        ...
        for ($y = 8; $y >= 1; $y--) {
            for ($x = 'a'; $x <= 'h'; $x++) {
                $res = new stdClass();
                $res->name = (isset($this->figures[$x][$y]) ? get_class($this->figures[$x][$y]) : null);
                $res->x = (isset($this->figures[$x][$y]) ? $x : null);
                $res->y = (isset($this->figures[$x][$y]) ? $y : null);
